...
bool excluirElemLista(LISTA *l, TIPOCHAVE ch)
{
  ...
  if (i == NULL)
    return false; // elemento a ser excluido nao existe
  ant->prox = i->prox;
  free(i);
  return true;
} /* excluirElemListaEnc */
...
void Josepheus(LISTA *l)
{
  ...
  while (tamanho(l) > 1)
  {
    ...
    if (excluir == l->cabeca)
    {
      ...
      ele->prox = excluir->prox;
    }
    free(excluir);
    if (ele->prox == l->cabeca) {
      ele = l->cabeca->prox;
    }
    else
    {
  ...
  TIPOCHAVE ch;
  retornarPrimeiro(l, &ch);
  printf("Para sobreviver você deve sentar na posição %i\n\n", ch);
}